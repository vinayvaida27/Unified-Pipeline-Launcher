...
.EXAMPLE
    .\scripts\update_dependencies.ps1 -Target launcher -Package "requests>=2.32,<3"

.EXAMPLE
    .\scripts\update_dependencies.ps1 -Target app -AppId 01_hello_pipeline -Package "plotly>=5,<6"
#>
...
if ($Package -ne "") {
    if ($Target -eq "launcher") {
        $RequirementFile = Join-Path $ReleaseDir "requirements-launcher.txt"
    } else {
